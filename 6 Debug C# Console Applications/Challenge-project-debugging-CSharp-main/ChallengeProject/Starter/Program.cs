...
int[,] registerDailyStartingCash = new int[,] { { 1, 5 }, { 5, 0 }, { 10, 0 }, { 20, 0 } };
...
while (transactions > 0)
{
    ...
    catch (InvalidOperationException e)
    {
        ...
        // correctTill(cashTill, paymentTwenties, paymentTens, paymentFives, paymentOnes);
    }
...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    int tillOnes = cashTill[0] + ones;
    int tillFives = cashTill[1] + fives;
    int tillTens = cashTill[2] + tens;
    int tillTwenties = cashTill[3] + twenties;

    if (changeNeeded < 0)
        throw new InvalidOperationException("InvalidOperationException: Not enough money provided to complete the transaction.");

    if (changeNeeded > TillAmount(cashTill))
    ...
    while ((changeNeeded > 19) && (tillTwenties > 0))
    {
        tillTwenties--;
        changeNeeded -= 20;
    ...
    while ((changeNeeded > 9) && (tillTens > 0))
    {
        tillTens--;
        changeNeeded -= 10;
    ...
    while ((changeNeeded > 4) && (tillFives > 0))
    {
        tillFives--;
        changeNeeded -= 5;
    ...
    while ((changeNeeded > 0) && (tillOnes > 0))
    {
        tillOnes--;
        changeNeeded -= 1;
    ...
    cashTill[0] = tillOnes;
    cashTill[1] = tillFives;
    cashTill[2] = tillTens;
    cashTill[3] = tillTwenties;
}
...
{
    return $"The till has {TillAmount(cashTill)} dollars";
}

static int TillAmount(int[] cashTill)
{
    return cashTill[3] * 20 + cashTill[2] * 10 + cashTill[1] * 5 + cashTill[0];
}
